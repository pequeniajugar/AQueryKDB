...
.aq.q0:{
 ...
 .aq.initQueryState[];
 aq__t1:.aq.initTable[nosplit_avgpx;"nosplit_avgpx";0b];
 aq__t1:.aq.sort[aq__t1;((iasc;{x^.aq.cd x} `Id);(iasc;{x^.aq.cd x} `year));({x^.aq.cd x} `Id;{x^.aq.cd x} `year;{x^.aq.cd x} `avg_px)];
 aq__t1:?[aq__t1;();0b;((`Id);(`year);(`avg_px))!({x^.aq.cd x} `Id;{x^.aq.cd x} `year;{x^.aq.cd x} `avg_px)];
 aq__t1
 };
.aq.q0[]
