...
.aq.q0:{
 .aq.initQueryState[];
 aq__t0:.aq.initTable[Target;"Target";0b];
 aq__t0:?[aq__t0;();(({x^.aq.cd x} `Id);({x^.aq.cd x} `timeBucket))!({x^.aq.cd x} `Id;(getYear;{x^.aq.cd x} `TradeDate));({x^.aq.cd x} `Id;{x^.aq.cd x} `timeBucket) _ (.aq.wildCard aq__t0)];
 aq__t0:?[aq__t0;();0b;((`Id);(`timeBucket);(`c__0);(`c__1);(`c__2);(`c__3))!({x^.aq.cd x} `Id;{x^.aq.cd x} `timeBucket;`$"yearly";(.aq.min';{x^.aq.cd x} `ClosePrice);(.aq.max';{x^.aq.cd x} `ClosePrice);(.aq.avg';{x^.aq.cd x} `ClosePrice))];
 aq__t0
 };
.aq.q0[]
