...
.aq.show `orders set ([]ordernum:"j"$();itemnum:"j"$();quantity:"j"$();price:"f"$();storeid:"s"$();vendorid:"s"$());

.aq.show `store set ([]storeid:"s"$();name:"s"$());

.aq.show `item set ([]itemnum:"j"$();price:"f"$());

.aq.show `vendorOutstanding set ([]vendorid:"s"$();amount:"f"$());

.aq.show `storeOutstanding set ([]storeid:"s"$();amount:"f"$());

// verbatim code

.bench.readLines:{[file] @[read0;`$file;{()}]};

itemFile:"/Users/tianxin/projects/nyu/ms1/research/dbtunning_experiements/data_generation/store/item_fractal_10_7.csv";
itemLines:.bench.readLines itemFile;
if[0<count itemLines;
  itemParsed:"," vs' 1_ itemLines;
  item,:flip `itemnum`price!(
    "J"$ first each itemParsed;
    "F"$ itemParsed[;1]
    )
  ];

storeFile:"/Users/tianxin/projects/nyu/ms1/research/dbtunning_experiements/data_generation/store/store_fractal_10_7.csv";
storeLines:.bench.readLines storeFile;
if[0<count storeLines;
  storeParsed:"," vs' 1_ storeLines;
  store,:flip `storeid`name!(
    `$ first each storeParsed;
    `$ storeParsed[;1]
    )
  ];

vendorOutstandingFile:"/Users/tianxin/projects/nyu/ms1/research/dbtunning_experiements/data_generation/store/vendorOutstanding_fractal_10_7.csv";
vendorOutstandingLines:.bench.readLines vendorOutstandingFile;
if[0<count vendorOutstandingLines;
  vendorOutstandingParsed:"," vs' 1_ vendorOutstandingLines;
  vendorOutstanding,:flip `vendorid`amount!(
    `$ first each vendorOutstandingParsed;
    "F"$ vendorOutstandingParsed[;1]
    )
  ];
storeOutstandingFile:"/Users/tianxin/projects/nyu/ms1/research/dbtunning_experiements/data_generation/store/storeOutstanding_fractal_10_7.csv";
storeOutstandingLines:.bench.readLines storeOutstandingFile;
if[0<count storeOutstandingLines;
  storeOutstandingParsed:"," vs' 1_ storeOutstandingLines;
  storeOutstanding,:flip `storeid`amount!(
    `$ first each storeOutstandingParsed;
    "F"$ storeOutstandingParsed[;1]
    )
  ];

ordersFile:"/Users/tianxin/projects/nyu/ms1/research/dbtunning_experiements/data_generation/store/order_fractal_10_7.csv";
ordersLines:.bench.readLines ordersFile;
if[0<count ordersLines;
  ordersParsed:"," vs' 1_ ordersLines;
  orders,:flip `ordernum`itemnum`quantity`price`storeid`vendorid!(
    "J"$ ordersParsed[;0];
    "J"$ ordersParsed[;1];
    "J"$ ordersParsed[;2];
    "F"$ ordersParsed[;3];
...
.trg.gen_updateVendorOutstanding_0:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`updateVendorOutstanding];
 ctx:.trg.ctxSet[ctx;`triggerTable;`orders];
 ctx:.trg.ctxSet[ctx;`triggerEvent;`insert];
 ctx:.trg.ctxSet[ctx;`triggerTiming;`after];
 ctx:.trg.ctxSet[ctx;`triggerRefs;"NEW TABLE AS inserted"];
  .aq.initQueryState[];
 inserted:.aq.initTable[.trg.rows ctx;"inserted";0b];
  aq__t0:.aq.initTable[vendorOutstanding;"vendorOutstanding";1b];
 aq__t1:.aq.initTable[inserted;"inserted";1b];
 aq__t2:aq__t0 cross aq__t1;
 aq__t2:?[aq__t2;enlist ((=; {x^.aq.cd x} `vendorOutstanding.vendorid; {x^.aq.cd x} `inserted.vendorid));0b;()];
 aq__t2:?[aq__t2;();0b;((`vendorid);(`amount))!({x^.aq.cd x} `vendorOutstanding.vendorid;(+; {x^.aq.cd x} `amount; (*; {x^.aq.cd x} `inserted.quantity; {x^.aq.cd x} `inserted.price)))];
 .trg.syncMappedUpdateRowsTo[aq__t2;`vendorOutstanding;enlist `vendorid;enlist `vendorid;(enlist `amount)!enlist `amount];
 ctx
 };
 .trg.register[`orders;`insert;`after;`updateVendorOutstanding;100;`.trg.gen_updateVendorOutstanding_0];

.trg.gen_updateStoreOutstanding_1:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`updateStoreOutstanding];
 ctx:.trg.ctxSet[ctx;`triggerTable;`orders];
 ctx:.trg.ctxSet[ctx;`triggerEvent;`insert];
 ctx:.trg.ctxSet[ctx;`triggerTiming;`after];
 ctx:.trg.ctxSet[ctx;`triggerRefs;"NEW TABLE AS inserted"];
  .aq.initQueryState[];
 inserted:.aq.initTable[.trg.rows ctx;"inserted";0b];
  aq__t0:.aq.initTable[storeOutstanding;"storeOutstanding";1b];
 aq__t1:.aq.initTable[inserted;"inserted";1b];
 aq__t2:aq__t0 cross aq__t1;
 aq__t2:?[aq__t2;enlist ((=; {x^.aq.cd x} `storeOutstanding.storeid; {x^.aq.cd x} `inserted.storeid));0b;()];
 aq__t2:?[aq__t2;();0b;((`storeid);(`amount))!({x^.aq.cd x} `storeOutstanding.storeid;(+; {x^.aq.cd x} `amount; (*; {x^.aq.cd x} `inserted.quantity; {x^.aq.cd x} `inserted.price)))];
 .trg.syncMappedUpdateRowsTo[aq__t2;`storeOutstanding;enlist `storeid;enlist `storeid;(enlist `amount)!enlist `amount];
 ctx
 };
 .trg.register[`orders;`insert;`after;`updateStoreOutstanding;100;`.trg.gen_updateStoreOutstanding_1];
