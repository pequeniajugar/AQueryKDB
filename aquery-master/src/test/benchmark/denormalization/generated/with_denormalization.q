...
.aq.q0:{
 .aq.initQueryState[];
 aq__t0:.aq.initTable[lineitemdenormalized;"lineitemdenormalized";0b];
 aq__t0:?[aq__t0;enlist ((=; {x^.aq.cd x} `r_region; `$"EUROPE"));0b;()];
 aq__t0:?[aq__t0;();0b;((`l_orderkey);(`l_partkey);(`l_suppkey);(`l_linenumber);(`l_quantity);(`l_extendedprice);(`l_discount);(`l_tax);(`l_returnflag);(`l_linestatus);(`l_shipdate);(`l_commitdate);(`l_receiptdate);(`l_shipinstruct);(`l_shipmode);(`l_comment);(`r_region))!({x^.aq.cd x} `l_orderkey;{x^.aq.cd x} `l_partkey;{x^.aq.cd x} `l_suppkey;{x^.aq.cd x} `l_linenumber;{x^.aq.cd x} `l_quantity;{x^.aq.cd x} `l_extendedprice;{x^.aq.cd x} `l_discount;{x^.aq.cd x} `l_tax;{x^.aq.cd x} `l_returnflag;{x^.aq.cd x} `l_linestatus;{x^.aq.cd x} `l_shipdate;{x^.aq.cd x} `l_commitdate;{x^.aq.cd x} `l_receiptdate;{x^.aq.cd x} `l_shipinstruct;{x^.aq.cd x} `l_shipmode;{x^.aq.cd x} `l_comment;{x^.aq.cd x} `r_region)];
 aq__t0
 };
.aq.q0[]
