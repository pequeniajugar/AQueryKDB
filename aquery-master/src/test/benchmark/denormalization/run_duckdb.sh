#!/bin/bash
# Benchmark: denormalized TPCH lineitem-region lookup on DuckDB.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

duckdb_db="${DUCKDB_DB:-/Users/tianxin/projects/nyu/ms2/independent_study/duckdb/tpch10_5.duckdb}"
output_csv_name="duckdb_big_denormalization.csv"
output_csv="./results/$output_csv_name"

mkdir -p "$(dirname "$output_csv")"
echo "dbms,label,iteration,execution_time,response_time" > "$output_csv"

echo "=== DuckDB Denormalization Benchmark ==="
echo "Database: $duckdb_db"
...
echo "DuckDB denormalization benchmark finished. Results saved to: $output_csv"
