...
.aq.show `table1 set ([]id:"j"$();qty:"j"$();note:"s"$());

.aq.show `table2 set ([]id:"j"$();qty:"j"$();note:"s"$());

.aq.insert[`table1;table1;();eval (enlist;1;10;enlist `$"src")]

.aq.insert[`table1;table1;();eval (enlist;2;20;enlist `$"src2")]

.aq.insert[`table2;table2;();eval (enlist;1;0;enlist `$"dest")]

.aq.insert[`table2;table2;();eval (enlist;2;0;enlist `$"dest2")]

.trg.gen_trg_sync_table2_expr_0:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`trg_sync_table2_expr];
 ...
 ctx:.trg.ctxSet[ctx;`triggerRefs;"NEW TABLE AS inserted"];
  .aq.initQueryState[];
 inserted:.aq.initTable[.trg.affectedResultRows ctx;"inserted";0b];
  aq__t0:.aq.initTable[inserted;"inserted";0b];
 aq__t0:?[aq__t0;();0b;((`id);(`qty);(`note))!({x^.aq.cd x} `inserted.id;(+; {x^.aq.cd x} `inserted.qty; 1);{x^.aq.cd x} `inserted.note)];
 .trg.syncMappedUpdateRowsTo[aq__t0;`table2;`id;`id;`qty`note!`qty`note];
 ctx
 };
 .trg.register[`table1;`update;`after;`trg_sync_table2_expr;100;`.trg.gen_trg_sync_table2_expr_0];

{
 .aq.initQueryState[];
 aq__t0:.aq.initTable[table1;"table1";0b];
 
 .aq.update[`table1;aq__t0;enlist ((=; {x^.aq.cd x} `id; 1));0b;(({x^.aq.cd x} `qty);({x^.aq.cd x} `note))!(42;enlist `$"src_updated")]
 }[];
