...
.aq.update:{[tnm;target;whereCode;byCode;setCode]
  ...
  uctx:.trg.run[tnm;`update;`before;uctx];
  ...
  result:.trg.applyNativeUpdate[tbl;whereObj;byObj;upd];
  assigned:tnm set result;
  .trg.run[tnm;`update;`after;(`target`where`by`set`result)!(uctx`target;uctx`where;uctx`by;uctx`set;result)];
  assigned
  };

.aq.deleteRows:{[tnm;target;whereCode]
  dctx:(`target`where)!(target;whereCode);
  dctx:.trg.run[tnm;`delete;`before;dctx];
  ...
  assigned:tnm set result;
  .trg.run[tnm;`delete;`after;(`target`where`result)!(dctx`target;dctx`where;result)];
  assigned
  };
...
.aq.show `table1 set ([]id:"j"$();subid:"j"$();qty:"j"$();note:"s"$());

.aq.show `table2 set ([]id:"j"$();subid:"j"$();qty:"j"$();note:"s"$());

.aq.insert[`table1;table1;();eval (enlist;1;10;100;enlist `$"src_a")]

.aq.insert[`table1;table1;();eval (enlist;1;20;200;enlist `$"src_b")]

.aq.insert[`table1;table1;();eval (enlist;2;10;300;enlist `$"src_c")]

.aq.insert[`table2;table2;();eval (enlist;1;10;0;enlist `$"dest_a")]

.aq.insert[`table2;table2;();eval (enlist;1;20;0;enlist `$"dest_b")]

.aq.insert[`table2;table2;();eval (enlist;2;10;0;enlist `$"dest_c")]

.trg.gen_trg_sync_table2_multikey_0:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`trg_sync_table2_multikey];
 ctx:.trg.ctxSet[ctx;`triggerTable;`table1];
 ctx:.trg.ctxSet[ctx;`triggerEvent;`update];
 ctx:.trg.ctxSet[ctx;`triggerTiming;`after];
 ctx:.trg.ctxSet[ctx;`triggerRefs;"NEW TABLE AS inserted"];
 .trg.syncMappedUpdateRowsTo[.trg.affectedResultRows ctx;`table2;`id`subid;`id`subid;`qty`note!`qty`note];
 ctx
 };
 .trg.register[`table1;`update;`after;`trg_sync_table2_multikey;100;`.trg.gen_trg_sync_table2_multikey_0];

{
 .aq.initQueryState[];
 aq__t0:.aq.initTable[table1;"table1";0b];
 
 .aq.update[`table1;aq__t0;((=; {x^.aq.cd x} `id; 1);(=; {x^.aq.cd x} `subid; 10));0b;(({x^.aq.cd x} `qty);({x^.aq.cd x} `note))!(111;enlist `$"src_a_updated")]
 }[];
