...
.aq.update:{[tnm;target;whereCode;byCode;setCode]
  ...
  oldRows:.trg.sourceRows[tbl;whereObj];
...
.aq.insert[`table1;table1;();eval (enlist;1;10;enlist `$"src_old")]
...
.trg.gen_trg_sync_table2_old_0:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`trg_sync_table2_old];
 ctx:.trg.ctxSet[ctx;`triggerTable;`table1];
 ctx:.trg.ctxSet[ctx;`triggerEvent;`update];
 ctx:.trg.ctxSet[ctx;`triggerTiming;`after];
 ctx:.trg.ctxSet[ctx;`triggerRefs;"OLD TABLE AS deleted"];
  .aq.initQueryState[];
 deleted:.aq.initTable[.trg.oldRows ctx;"deleted";0b];
  aq__t0:.aq.initTable[deleted;"deleted";0b];
 aq__t0:?[aq__t0;();0b;((`id);(`qty);(`note))!({x^.aq.cd x} `deleted.id;{x^.aq.cd x} `deleted.qty;{x^.aq.cd x} `deleted.note)];
 .trg.syncMappedUpdateRowsTo[aq__t0;`table2;`id;`id;`qty`note!`qty`note];
 ctx
 };
 .trg.register[`table1;`update;`after;`trg_sync_table2_old;100;`.trg.gen_trg_sync_table2_old_0];

{
 .aq.initQueryState[];
 aq__t0:.aq.initTable[table1;"table1";0b];
 
 .aq.update[`table1;aq__t0;enlist ((=; {x^.aq.cd x} `id; 1));0b;(({x^.aq.cd x} `qty);({x^.aq.cd x} `note))!(42;enlist `$"src_new")]
 }[];
