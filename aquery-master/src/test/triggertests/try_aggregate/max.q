...
.aq.show `orders set ([]id:"j"$();qty:"j"$());

.aq.show `max_log set ([]max_qty:"j"$());

.trg.gen_trg_max_after_insert_0:{[tbl;evt;ctx]
 ctx:.trg.ctxSet[ctx;`triggerName;`trg_max_after_insert];
 ...
 aq__t0:?[aq__t0;();0b;(enlist[`max_qty]!enlist (.aq.max;{x^.aq.cd x} `qty))];
 aq__insert_src:aq__t0;
 .aq.insert[`max_log;max_log;enlist `max_qty;aq__insert_src]
 ctx
 };
 .trg.register[`orders;`insert;`after;`trg_max_after_insert;100;`.trg.gen_trg_max_after_insert_0];

.aq.insert[`orders;orders;();eval (enlist;1;10)]

.aq.insert[`orders;orders;();eval (enlist;2;20)]
