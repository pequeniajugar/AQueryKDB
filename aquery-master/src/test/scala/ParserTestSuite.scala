...
class ParserTestSuite extends FunSuite {
  ...
  test("trigger") {
    val t1S = "CREATE TRIGGER trg_ins AFTER INSERT ON orders DO UPDATE orders SET qty = 0"
    val t1E = Trigger(
      "trg_ins",
      After,
      InsertEvent,
      "orders",
      Nil,
      Update("orders", ("qty", IntLit(0)) :: Nil, Nil, Nil, Nil, Nil)
    )
    assert(expectParse(trigger, t1S)(Some(t1E == _)), "parse minimal statement-level trigger")

    val t2S = """
      CREATE TRIGGER trg_upd
      AFTER UPDATE ON orders
      REFERENCING OLD TABLE AS deleted NEW TABLE AS inserted
      DO SELECT * FROM inserted
    """
    val t2E = Trigger(
      "trg_upd",
      After,
      UpdateEvent,
      "orders",
      List(TriggerRef(OldTableRef, "deleted"), TriggerRef(NewTableRef, "inserted")),
      Query(Nil, Project(Table("inserted", None), (WildCard, None) :: Nil))
    )
    assert(expectParse(trigger, t2S)(Some(t2E == _)), "parse trigger with transition tables")

    val d1S = "DROP TRIGGER trg_upd"
    val d1E = DropTrigger("trg_upd")
    assert(expectParse(dropTrigger, d1S)(Some(d1E == _)), "parse drop trigger")
  }
