...
class KdbGenerator extends BackEnd {
  ...
  case class TransitionUpdatePlan(
    destTable: String,
    transitionAlias: String,
    transitionKind: TriggerRefKind,
    destKeys: List[String],
    filters: List[Expr],
    setExprs: List[(String, Expr)]
  )

  case class TransitionDeletePlan(
    destTable: String,
    transitionAlias: String,
    transitionKind: TriggerRefKind,
    destKeys: List[String],
   ...
   * @return
   */
  def genLocalQueryWithInit(l: (String, List[String], RelAlg), initState: Boolean): CodeGen = l match {
    case (n, cols, plan) =>
      genPlanInternal(plan, (t: String) => s"$n:${genRelabelCols(cols, t)};", initState)
  }

  def genLocalQuery(l: (String, List[String], RelAlg)): CodeGen = genLocalQueryWithInit(l, initState = true)
   ...
   * @return
   */
  def genPlanInternal(plan: RelAlg, f: String => String, initState: Boolean): CodeGen =
    for(
      _ <- cleanQueryEnv;
      code <- genRelAlg(plan);
      t <- getCurrTable
    ) yield {
      val prefix = if (initState) s"$initQueryState\n" else ""
      s"$prefix$code\n ${f(t)}"
    }

  def genPlan(plan: RelAlg, f: String => String): CodeGen =
    genPlanInternal(plan, f, initState = true)
  ...
  def genQueryToVar(q: Query, resultVar: String): CodeGen = {
    val locals = SimpleState.sequence(q.local.map(genLocalQuery)).map(_.mkString("\n"))
    val main = genPlan(q.main, (t: String) => s"$resultVar:$t;")
    for (
      ls <- locals;
      m <- main
    ) yield if (q.local.nonEmpty) s"$ls\n$m" else s"$m"
  ...
  def getTriggerEventName(e: TriggerEvent): String = e match {
    case InsertEvent => "insert"
    case UpdateEvent => "update"
    case DeleteEvent => "delete"
  }

  def getTriggerTimingName(t: TriggerTime): String = t match {
    case Before => "before"
    case After => "after"
  }

  def getTriggerFuncName(name: String, id: Int): String = s".trg.gen_${name}_$id"

  def getTriggerRefKindByAlias(t: Trigger): Map[String, TriggerRefKind] =
    t.refs.map(r => r.alias -> r.kind).toMap

  def usesTransitionAlias(e: Expr, aliases: Set[String]): Boolean =
    e.allSubExpressions.exists {
      case ColumnAccess(tn, _) if aliases.contains(tn) => true
      case _ => false
    }

  def usesTransitionAlias(u: Update, aliases: Set[String]): Boolean =
    u.assigns.exists(a => usesTransitionAlias(a._2, aliases)) ||
      u.where.exists(usesTransitionAlias(_, aliases)) ||
      u.groupby.exists(usesTransitionAlias(_, aliases)) ||
      u.having.exists(usesTransitionAlias(_, aliases))

  def exprUsesOnlyAlias(e: Expr, alias: String): Boolean = e.allSubExpressions.forall {
    case _: Lit => true
    case ColumnAccess(tn, _) => tn == alias
    case _: BinExpr => true
    case _: UnExpr => true
    case _: FunCall => true
    case _: Case => true
  ...
  def exprUsesOnlyTables(e: Expr, allowed: Set[String]): Boolean = e.allSubExpressions.forall {
    case _: Lit => true
    case ColumnAccess(tn, _) => allowed.contains(tn)
    case _: Id => true
    case _: BinExpr => true
    case _: UnExpr => true
    case _: FunCall => true
  ...
  def extractTransitionAssign(
    destTable: String,
    aliases: Map[String, TriggerRefKind],
    assign: (String, Expr)
  ): Option[(String, String, TriggerRefKind, Expr)] = {
    val aliasMatches = assign._2.allSubExpressions.collect {
      case ColumnAccess(alias, _) if aliases.contains(alias) => alias -> aliases(alias)
    }.distinct
    aliasMatches match {
      case (alias, kind) :: Nil if exprUsesOnlyTables(assign._2, Set(destTable, alias)) =>
        Some((assign._1, alias, kind, assign._2))
      case _ => None
    }
  }

  def extractTransitionKeyPair(
    destTable: String,
    aliases: Map[String, TriggerRefKind],
    expr: Expr
  ): Option[(String, String, TriggerRefKind, String)] = expr match {
    case BinExpr(Eq, ColumnAccess(dt, dk), ColumnAccess(alias, sk)) if dt == destTable =>
      aliases.get(alias).map(kind => (dk, alias, kind, sk))
    case BinExpr(Eq, ColumnAccess(alias, sk), ColumnAccess(dt, dk)) if dt == destTable =>
      aliases.get(alias).map(kind => (dk, alias, kind, sk))
    case _ => None
  }

  def toTransitionUpdatePlan(t: Trigger, u: Update): Option[TransitionUpdatePlan] = {
    if (t.time != After) return None
    val aliases = getTriggerRefKindByAlias(t)
    if (aliases.isEmpty) return None

    val assignInfo = u.assigns.map(extractTransitionAssign(u.t, aliases, _))
    if (assignInfo.exists(_.isEmpty)) return None
    if (u.groupby.nonEmpty || u.having.nonEmpty || u.order.nonEmpty) return None

    val assignValues = assignInfo.flatten
    val keyInfo = u.where.flatMap(extractTransitionKeyPair(u.t, aliases, _))
    if (keyInfo.isEmpty) return None
    val keyAliases = keyInfo.map(_._2).distinct
    if (keyAliases.length != 1) return None
    val alias = keyAliases.head
    if (u.where.exists(e => !exprUsesOnlyTables(e, Set(u.t, alias)))) return None
    val whereValues = keyInfo
    val aliasesUsed = (assignValues.map(_._2) ++ whereValues.map(_._2)).distinct
    val kindsUsed = (assignValues.map(_._3) ++ whereValues.map(_._3)).distinct
    if (aliasesUsed.length != 1 || kindsUsed.length != 1) return None

    Some(TransitionUpdatePlan(
      destTable = u.t,
      transitionAlias = aliasesUsed.head,
      transitionKind = kindsUsed.head,
      destKeys = whereValues.map(_._1).distinct,
      filters = u.where,
      setExprs = assignValues.map { case (dc, _, _, expr) => dc -> expr }
    ))
  }

  def toTransitionDeletePlan(t: Trigger, d: Delete): Option[TransitionDeletePlan] = {
    if (t.time != After) return None
    val aliases = getTriggerRefKindByAlias(t)
    if (aliases.isEmpty) return None
    if (d.order.nonEmpty || d.groupby.nonEmpty || d.having.nonEmpty) return None
    val where = d.del match {
      case Left(_) => Nil
      case Right(fs) => fs
    }
    if (where.isEmpty) return None
    val whereValues = where.flatMap(extractTransitionKeyPair(d.t, aliases, _))
    if (whereValues.isEmpty) return None
    val aliasesUsed = whereValues.map(_._2).distinct
    if (aliasesUsed.length != 1) return None
    val alias = aliasesUsed.head
    if (where.exists(e => !exprUsesOnlyTables(e, Set(d.t, alias)))) return None
    val kindsUsed = whereValues.map(_._3).distinct
    if (aliasesUsed.length != 1 || kindsUsed.length != 1) return None
    Some(TransitionDeletePlan(
      destTable = d.t,
      transitionAlias = aliasesUsed.head,
      transitionKind = kindsUsed.head,
      destKeys = whereValues.map(_._1).distinct,
  ...
  def triggerRowsExpr(t: Trigger, kind: TriggerRefKind): Option[String] =
    (kind, t.event, t.time) match {
      case (NewTableRef, InsertEvent, _) => Some(".trg.rows ctx")
      case (OldTableRef, UpdateEvent, Before) => Some(".trg.oldRows ctx")
      case (NewTableRef, UpdateEvent, After) => Some(".trg.affectedResultRows ctx")
      case (OldTableRef, UpdateEvent, After) => Some(".trg.oldRows ctx")
      case (OldTableRef, DeleteEvent, Before) => Some(".trg.oldRows ctx")
      case (OldTableRef, DeleteEvent, After) => Some(".trg.oldRows ctx")
      case _ => None
    }

  def toKdbSymList(xs: List[String]): String = xs match {
    case Nil => "()"
    case ys => makeList(ys.map(kdbSym).mkString(""), ys.length)
  }

  def toKdbSymDict(pairs: List[(String, String)]): String =
    pairs match {
      case (dk, sk) :: Nil => s"(enlist ${kdbSym(dk)})!enlist ${kdbSym(sk)}"
      case _ => s"${toKdbSymList(pairs.map(_._1))}!${toKdbSymList(pairs.map(_._2))}"
    }

  def getTriggerRefRowsExpr(kind: TriggerRefKind, event: TriggerEvent, timing: TriggerTime): Option[String] =
    (kind, event, timing) match {
      case (NewTableRef, InsertEvent, _) => Some(".trg.rows ctx")
      case (OldTableRef, UpdateEvent, Before) => Some(".trg.oldRows ctx")
      case (NewTableRef, UpdateEvent, After) => Some(".trg.affectedResultRows ctx")
      case (OldTableRef, UpdateEvent, After) => Some(".trg.oldRows ctx")
  ...
  def genTriggerRefBindings(t: Trigger): String =
    t.refs.flatMap { ref =>
      getTriggerRefRowsExpr(ref.kind, t.event, t.time).map { rowsExpr =>
        s"""${ref.alias}:.aq.initTable[$rowsExpr;"${ref.alias}";0b];"""
      }
    }.mkString("\n ")

  def injectAfterInit(body: String, injected: String): String = {
    if (injected.isEmpty) body
    else {
      val marker = ".aq.initQueryState[];"
      val idx = body.indexOf(marker)
      if (idx < 0) injected + "\n" + body
      else {
        val insertAt = idx + marker.length
        body.substring(0, insertAt) + "\n " + injected + body.substring(insertAt)
      }
    }
  }

  def unwrapImmediateBlock(body: String): String = {
  ...
  def genTransitionUpdateTrigger(
    t: Trigger,
    funcName: String,
    triggerName: String,
    tableName: String,
    ...
    timingCode: String,
    refsCode: String,
    plan: TransitionUpdatePlan
  ): CodeGen = {
    val rowsExpr = triggerRowsExpr(t, plan.transitionKind).getOrElse {
      throw new IllegalArgumentException("unsupported transition-table rows source for trigger")
    }
    val projections =
      plan.destKeys.map { destKey => (ColumnAccess(plan.destTable, destKey): Expr, Some(destKey)) } :::
        plan.setExprs.map { case (destCol, expr) => (expr, Some(destCol)) }
    val joined = Join(
      Cross,
      Table(plan.destTable, Some(plan.destTable)),
      Table(plan.transitionAlias, Some(plan.transitionAlias)),
      Nil
    )
    val projected = Project(Filter(joined, plan.filters), projections)
    for (
      _ <- cleanQueryEnv;
      rowCode <- genRelAlg(projected);
      rowTable <- getCurrTable
    ) yield {
      val destKeys = toKdbSymList(plan.destKeys)
      val colMap = toKdbSymDict(plan.setExprs.map { case (destCol, _) => destCol -> destCol })
      s"""$funcName:{[tbl;evt;ctx]
         | ctx:.trg.ctxSet[ctx;`triggerName;$triggerName];
         | ctx:.trg.ctxSet[ctx;`triggerTable;$tableName];
         | ctx:.trg.ctxSet[ctx;`triggerEvent;$eventCode];
         | ctx:.trg.ctxSet[ctx;`triggerTiming;$timingCode];
         | ctx:.trg.ctxSet[ctx;`triggerRefs;$refsCode];
         | ${initQueryState}
         | ${plan.transitionAlias}:.aq.initTable[$rowsExpr;"${plan.transitionAlias}";0b];
         | $rowCode
         | .trg.syncMappedUpdateRowsTo[$rowTable;${kdbSym(plan.destTable)};$destKeys;$destKeys;$colMap];
         | ctx
         | };
         | .trg.register[$tableName;$eventCode;$timingCode;$triggerName;100;${kdbSym(funcName)}];""".stripMargin
    }
  }

  def genTransitionDeleteTrigger(
    t: Trigger,
    funcName: String,
    triggerName: String,
    tableName: String,
    ...
    timingCode: String,
    refsCode: String,
    plan: TransitionDeletePlan
  ): CodeGen = {
    val rowsExpr = triggerRowsExpr(t, plan.transitionKind).getOrElse {
      throw new IllegalArgumentException("unsupported transition-table rows source for delete trigger")
    }
    val projections = plan.destKeys.map { destKey =>
      (ColumnAccess(plan.destTable, destKey): Expr, Some(destKey))
    }
    val joined = Join(
      Cross,
      Table(plan.destTable, Some(plan.destTable)),
    ...
    for (
      ...
      s"""$funcName:{[tbl;evt;ctx]
         ...
         | ${plan.transitionAlias}:.aq.initTable[$rowsExpr;"${plan.transitionAlias}";0b];
         | $rowCode
         | .trg.syncMappedDeleteRowsFrom[$rowTable;${kdbSym(plan.destTable)};$destKeys;$destKeys];
         | ctx
         | };
         | .trg.register[$tableName;$eventCode;$timingCode;$triggerName;100;${kdbSym(funcName)}];""".stripMargin
    }
  ...
  def genTriggerDefault(
    ...
    genTopLevel(t.body).map { body =>
      val boundBody = injectAfterInit(unwrapImmediateBlock(body), genTriggerRefBindings(t))
      s"""$funcName:{[tbl;evt;ctx]
         | ctx:.trg.ctxSet[ctx;`triggerName;$triggerName];
         | ctx:.trg.ctxSet[ctx;`triggerTable;$tableName];
         | ctx:.trg.ctxSet[ctx;`triggerEvent;$eventCode];
  ...
  def genTriggerInsertQueryDefault(
    t: Trigger,
    funcName: String,
    triggerName: String,
    tableName: String,
    ...
    val modifier: CodeGen =
      if (i.modifier.nonEmpty)
        SimpleState.unit(toKdbSymList(i.modifier))
      else
        kdbEmptyList
    for (
      queryCode <- genQueryToVar(q, "aq__insert_src");
      s <- sorted;
      m <- modifier
    ) yield {
      val body =
        s"""$queryCode
           | .aq.insert[${kdbSym(i.n)};$s;$m;aq__insert_src]""".stripMargin
      val boundBody = injectAfterInit(body, genTriggerRefBindings(t))
      s"""$funcName:{[tbl;evt;ctx]
         | ctx:.trg.ctxSet[ctx;`triggerName;$triggerName];
         | ctx:.trg.ctxSet[ctx;`triggerTable;$tableName];
         | ctx:.trg.ctxSet[ctx;`triggerEvent;$eventCode];
  ...
  def genDropTrigger(dt: DropTrigger): CodeGen =
    SimpleState.unit(s".trg.drop[${kdbSym(dt.name)}];")
  ...
  def genTrigger(t: Trigger): CodeGen = for (
    id <- getQueryId;
    _ <- incrQueryId;
    code <- {
      val funcName = getTriggerFuncName(t.name, id)
      val eventCode = kdbSym(getTriggerEventName(t.event))
      val timingCode = kdbSym(getTriggerTimingName(t.time))
      val triggerName = kdbSym(t.name)
      val tableName = kdbSym(t.table)
      val refsCode =
        if (t.refs.isEmpty) "()" else "\"" + t.refs.map {
          case TriggerRef(NewTableRef, alias) => s"NEW TABLE AS $alias"
          case TriggerRef(OldTableRef, alias) => s"OLD TABLE AS $alias"
        }.mkString(", ").replace("\"", "\\\"") + "\""
      t.body match {
        case u: Update =>
          toTransitionUpdatePlan(t, u) match {
            case Some(plan) =>
              genTransitionUpdateTrigger(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode, plan)
            case None =>
              genTriggerDefault(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode)
          }
        case d: Delete =>
          toTransitionDeletePlan(t, d) match {
            case Some(plan) =>
              genTransitionDeleteTrigger(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode, plan)
            case None =>
              genTriggerDefault(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode)
          }
        case i @ Insert(_, _, _, Right(q)) if t.refs.nonEmpty =>
          genTriggerInsertQueryDefault(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode, i, q)
        case _ =>
          genTriggerDefault(t, funcName, triggerName, tableName, eventCode, timingCode, refsCode)
      }
    }
      ...
      case Delete(t, Right(fs), _, groupby, having) =>
        for (
          ...
          having match {
            ...
            case Nil => s"{\n$init\n .aq.deleteRows[${kdbSym(t)};$target;$w]\n }[];"
            // if there is a having, we need to use a helper to solve for indices that need
            // to be deleted, and pass this to a delete op
            case _ => s"{\n$init\n$ix\n\n .aq.deleteRows[${kdbSym(t)};$target;enlist $vectorName]\n }[];"
          }
        }
    }
  }
  ...
  def genTopLevel(t: TopLevel): CodeGen = t match {
    ...
    case tr: Trigger => genTrigger(tr)
    case dt: DropTrigger => genDropTrigger(dt)
