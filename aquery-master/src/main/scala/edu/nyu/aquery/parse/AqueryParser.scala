...
object AqueryParser extends StandardTokenParsers with PackratParsers {
  ...
  protected val AFTER = Keyword("AFTER")
  ...
  protected val REFERENCING = Keyword("REFERENCING")
  ...
  protected val TRIGGER = Keyword("TRIGGER")
  ...
  def trigger: Parser[Trigger] =
    positioned(CREATE ~> TRIGGER ~> ident ~
      triggerTime ~
      triggerEvent ~
      (ON ~> ident) ~
      triggerRefs.? ~
      (DO ~> triggerBody) ^^ { case n ~ t ~ e ~ table ~ refs ~ body =>
      Trigger(n, t, e, table, refs.getOrElse(Nil), body)
    })

  def triggerTime: Parser[TriggerTime] =
    BEFORE ^^^ Before |
      AFTER ^^^ After

  def triggerEvent: Parser[TriggerEvent] =
    INSERT ^^^ InsertEvent |
      UPDATE ^^^ UpdateEvent |
      DELETE ^^^ DeleteEvent

  def triggerRefs: Parser[List[TriggerRef]] =
    REFERENCING ~> rep1(triggerRef)

  def triggerRef: Parser[TriggerRef] =
    ((NEW ~ TABLE) ~> (AS ~> ident) ^^ { alias => TriggerRef(NewTableRef, alias) }
      | (OLD ~ TABLE) ~> (AS ~> ident) ^^ { alias => TriggerRef(OldTableRef, alias) })

  def triggerBody: Parser[TopLevel] =
    update |
      delete |
      insert |
      fullQuery

  def dropTrigger: Parser[DropTrigger] =
    positioned(DROP ~> TRIGGER ~> ident ^^ { name => DropTrigger(name) })
